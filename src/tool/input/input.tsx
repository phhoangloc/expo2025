...
export const Input = ({ onChange, name, value, type, disabled, warn, icon1, icon2, sx, onFocus }: Props) => {
    ...
    return (
        <div className={`h-12 text-left relative mb-2 ${disabled ? "opacity-50" : ""} ${sx ? sx : ""}`}>
            <p className={` absolute transition-all duration-300 h-max px-2 opacity-75 text-main ${_focus || value ? "top-0 text-sm text-lv-13 opacity-50" : "top-4"}`}
            ...
            />
            <div className='w-max absolute flex right-1 bottom-0 h-12 z-[1] text-main'>
                {icon1}{icon2}
            </div>
            <div className={`w-full h-full absolute z-0 border-b top-0 left-0 border-main`}></div>
            <div className={`w-full h-full absolute z-0 border-b-2 top-0 left-0 transition-all duration-300 border-main ${_focus || value ? "scale-x-[100%]" : "scale-x-[0%]"}`}></div>
        </div>
    )
}
